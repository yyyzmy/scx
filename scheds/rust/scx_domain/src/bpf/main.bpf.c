...
static __always_inline s32 round_robin_select_llc_in_node(struct topo_ctx *topo, u32 node_id)
{
	...
	u32 node_nr_llcs = topo->nr_llcs / topo->nr_nodes;
	if (node_nr_llcs == 0) {
		bpf_printk("node_nr_llcs == 0");
		return -EINVAL;
	}
	u32 llc_start = node_id * node_nr_llcs;
	u32 next_llc_id;

	struct node_ctx *nodec = lookup_node_ctx(node_id);
	if (nodec) {
		next_llc_id = __sync_fetch_and_add(&nodec->last_llc_idx, 1);
		next_llc_id = llc_start + next_llc_id % node_nr_llcs;
	} else {
		next_llc_id = llc_start;
	}
...
static __always_inline s32 round_robin_select_cpu(struct topo_ctx *topo, u32 node_id, s32 cpu_allowed_flag)
{
	...
	u32 next_cpu_id;
	struct llc_ctx *llc = lookup_llc_ctx(llc_idx);
	if (!llc || llc->nr_cpus == 0) {
		bpf_printk("Invalid LLC or nr_cpus == 0");
		return -EINVAL;
	}

	u32 cpu_start = llc_idx * llc->nr_cpus;

	if (llc && llc->nr_cpus > 0) {
		next_cpu_id = __sync_fetch_and_add(&llc->last_cpu_id, 1);
		next_cpu_id = cpu_start + next_cpu_id % llc->nr_cpus;
	} else {
		next_cpu_id = cpu_start;
	}
...
s32 BPF_STRUCT_OPS(domain_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
	...
	if (wake_flags & SCX_WAKE_FORK) {
		u32 pid_parent = p->real_parent->pid;
...
void BPF_STRUCT_OPS(domain_enqueue, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(domain_dispatch, s32 cpu, struct task_struct *p)
{
...
static bool check_cpu_in_mask(int cpu, const u8 *cpumask)
{
	...
	if (byte_idx < 0 || byte_idx >= (max_cpus / 8)) {
		return false;
	}
...
static s32 init_llc_bpf_mask(u32 llc)
{
	...
	if (!(llcc = lookup_llc_ctx(llc))) {
		return -ENOENT;
	}

	ret = create_topo_cpumask(&llcc->bpf_cpumask);
	if (ret) {
		return ret;
	}

	bpf_rcu_read_lock();
	cpumask = llcc->bpf_cpumask;
	if (!cpumask) {
		bpf_rcu_read_unlock();
	...
	bpf_for(cpu, 0, max_cpus) {
		if (check_cpu_in_mask(cpu, llcc->cpumask)) {
...
int32_t BPF_STRUCT_OPS_SLEEPABLE(domain_init)
{
	int i, ret;
	uint32_t key = 0;
	struct topo_ctx *topo;
...
void BPF_STRUCT_OPS(domain_runnable, struct task_struct *p)
{
...
void BPF_STRUCT_OPS(domain_quiescent, struct task_struct *p)
{
...
void BPF_STRUCT_OPS(domain_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(domain_ops,
		.select_cpu		= (void *)domain_select_cpu,
		.enqueue 		= (void *)domain_enqueue,
		.dispatch 		= (void *)domain_dispatch,
		.runnable		= (void *)domain_runnable,
		.quiescent		= (void *)domain_quiescent,
		.init			= (void *)domain_init,
		.exit			= (void *)domain_exit,
		.name			= "domain");
